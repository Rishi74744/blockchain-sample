...
describe('Block', () => {
    ...
    const lastHash = 'lasthash-t';
    const hash = 'hash-t';
    const data = ['blockchain', 'data', 't'];
    const nonce = 1;
    const difficulty = 1;
    ...
    describe('adjustDifficulty()', () => {
        ...
        it('raise the difficulty for a quickly minde block', () => {
            expect(Block.adjustDifficulty({
                originalBlock: block, timestamp: block.timestamp + MINE_RATE - 100
            })).toEqual(block.difficulty + 1);
        });

        it('lowers the difficulty for a quickly minde block', () => {
            expect(Block.adjustDifficulty({
                originalBlock: block, timestamp: block.timestamp + MINE_RATE + 100
            })).toEqual(block.difficulty - 1);
        });
